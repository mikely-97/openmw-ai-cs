"""CLOT — Clothing record.

Subrecords:
  NAME  → record_id (RefId)
  MODL  → mesh path
  FNAM  → display name
  CTDT  → clothing data (12 bytes: int32 type + float weight + uint16 value + uint16 enchant_pts)
  SCRI  → script RefId
  ITEX  → icon path
  INDX  → part index (1 byte)
  BNAM  → male body part mesh RefId
  CNAM  → female body part mesh RefId
...
from omwtools.records.armo import PartRef

CTDT_FMT = "<ifHH"
CTDT_SIZE = struct.calcsize(CTDT_FMT)  # 12


@dataclass
class Clothing(BaseRecord):
    """CLOT record — clothing item."""
    ...
    @classmethod
    def from_raw(cls, raw: RawRecord, format_version: int) -> "Clothing":
        ...
        ctdt = raw.get_subrecord(b"CTDT")
        if ctdt and len(ctdt.data) >= CTDT_SIZE:
            obj.clot_type, obj.weight, obj.value, obj.enchant_pts = struct.unpack_from(CTDT_FMT, ctdt.data)
    ...
    def encode_subrecords(self, format_version: int) -> bytes:
        ...
        out += pack_subrec_header(b"CTDT", CTDT_SIZE)
        out += struct.pack(CTDT_FMT, self.clot_type, self.weight, self.value, self.enchant_pts)
